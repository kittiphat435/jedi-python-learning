    ...
    try {
        ...
        switch (problemData.type) {
            ...
            case 'python':
                ...
                const testCasesList = document.getElementById('pythonSection');
                    ...
                    problemData.testCases.forEach(testCase => {
                        ...
                        testCaseDiv.innerHTML = `
                            <div class="test-case-content">
                                ...
                                <div class="form-group">
                                    <label>Output *</label>
                                    <input type="text" class="test-output input-field" value="${testCase.expected || ''}" placeholder="เช่น 15" required>
                                </div>
                                <div class="form-group">
                                    <label>คะแนน</label>
                                    <input type="number" class="test-score input-field" value="${testCase.score || 1}" min="1" max="10">
            ...
            case 'matching':
                ...
                // ใส่คำอธิบายโจทย์
                if (matchingDescription) matchingDescription.value = problemData.description || '';
                
                // ฟังก์ชันช่วยสร้างตัวเลือก Dropdown
                const createOptions = (count, selected) => {
                    let html = '';
                ...
                // --- เตรียมข้อมูล (รองรับทั้งแบบเก่าและใหม่) ---
                let questions = [], answers = [], pairs = [];

                // แบบใหม่: แยกเก็บ questions/answers/pairs
                if (Array.isArray(problemData.questions)) questions = problemData.questions;
                if (Array.isArray(problemData.answers)) answers = problemData.answers;
                if (Array.isArray(problemData.pairs)) pairs = problemData.pairs;

                // แบบเก่า: เก็บข้อความไว้ใน pairs โดยตรง (แปลงข้อมูลให้เข้ากับแบบใหม่)
                if (questions.length === 0 && pairs.length > 0 && typeof pairs[0].question === 'string') {
                    pairs.forEach(p => {
                        if (!questions.includes(p.question)) questions.push(p.question);
                        if (!answers.includes(p.answer)) answers.push(p.answer);
                    });
                    // แปลงคู่จับคู่ให้เก็บเป็น Index แทนข้อความ
                    pairs = pairs.map(p => ({
                        questionIndex: questions.indexOf(p.question),
                        answerIndex: answers.indexOf(p.answer),
                        score: p.score || 1
                    }));
                }
                ...
                if (matchingQuestionsList) {
                    ...
                    questions.forEach((q, i) => {
                        matchingQuestionsList.insertAdjacentHTML('beforeend', `
                            <div class="matching-item">
                                <span class="item-number">${i + 1}.</span>
                                <input type="text" class="matching-question input-field" value="${q}" required>
                                <button type="button" class="delete-btn" onclick="this.parentElement.remove()">ลบ</button>
                            </div>`);
                    });
                }
                ...
                if (matchingAnswersList) {
                    ...
                    answers.forEach((a, i) => {
                        matchingAnswersList.insertAdjacentHTML('beforeend', `
                            <div class="matching-item">
                                <span class="item-number">${i + 1}.</span>
                                <input type="text" class="matching-answer input-field" value="${a}" required>
                                <button type="button" class="delete-btn" onclick="this.parentElement.remove()">ลบ</button>
                            </div>`);
                    });
                }

                // 4. แสดงรายการ "คู่จับคู่" (พร้อมช่องคะแนน!)
    ...
    try {
        if (problemType === 'gui') {
            ...
            const testCases = Array.from(document.querySelectorAll('#guiTestCasesList .test-case')).map(testCase => {
                ...
                // ตรวจสอบว่ามี actions และ outputs ครบถ้วนหรือไม่
                if (actions.length === 0 || outputs.length === 0) {
                    console.warn('Test Case ไม่สมบูรณ์: ขาด actions หรือ outputs');
                    return null;
                }
            ...
            if (testCases.length === 0) {
                alert('กรุณาเพิ่ม Test Case ที่มี Actions และ Outputs ครบถ้วนก่อนบันทึกโจทย์');
                return;
            }

            // 1. คะแนนจาก Widgets
            const widgetScore = widgets.reduce((sum, w) => sum + (parseInt(w.score) || 1), 0);
            
            // 2. คะแนน Order (ให้ 5 คะแนน ถ้ามี widget >= 2 ตัว)
            const orderScore = (widgets.length >= 2) ? 5 : 0;
            
            // 3. คะแนน Test Cases
            const testCaseScore = testCases.reduce((sum, tc) => sum + (parseInt(tc.score) || 1), 0);
            
            // รวมกันเป็นคะแนนเต็มของโจทย์ข้อนี้
            const totalMaxScore = widgetScore + orderScore + testCaseScore;
            ...
            Object.assign(problemData, {
                ...
                widgets,
                testCases,
                maxScore: totalMaxScore, // ✅ บันทึกค่านี้ลง Database!
                templateWidgetCount: countWidgets(templateCode),
                solutionWidgetCount: countWidgets(solutionCode)
            });
        } else if (problemType === 'python') {
            const description = document.getElementById('pythonDescription')?.value?.trim() || '';
            const templateCode = window.editor?.getValue() || '';
            const solutionCode = window.solutionEditor?.getValue() || '';
            ...
            const testCases = Array.from(document.querySelectorAll('#testCasesList .test-case')).map(testCase => {
                ...
                // แยก input เป็น array ถ้ามีหลายบรรทัด
                const inputs = input.split('\n').map(line => line.trim()).filter(line => line);
                
                return inputs.length && expected ? { 
                    input,
                    inputs, 
                    expected, 
                    score, 
            ...
            if (testCases.length === 0) {
                alert('กรุณาเพิ่ม Test Case ที่มี Input และ Output ครบถ้วนก่อนบันทึกโจทย์');
                return;
            }
    ...
    for (const line of lines) {
        ...
        try {
            ...
            // Label
                ...
                const font = trimmedLine.match(/font=\((.*?)\)/)?.[1];

                let style = `color:${fg};`;
                if (font) {
                    const [family, size, ...options] = eval(`[${font}]`);
                    style += `font-family:${family};font-size:${size}px;`;
                    if (options.includes('bold')) style += 'font-weight:bold;';
                }

                elements.push(`<div style="${style}">${text}</div>`);
            }
            // Button
...
function validateGUICode(code) {
    ...
    try {
        ...
        if (!code.includes('root = Tk()')) {
            throw new Error('โค้ดต้องมี root = Tk() เพื่อสร้างหน้าต่างหลัก');
        }
        if (!code.includes('root.mainloop()')) {
            throw new Error('โค้ดต้องมี root.mainloop() เพื่อรันโปรแกรม');
        }
        ...
        for (const testCase of testCases) {
            ...
            if (!testCase.outputs.length) {
                throw new Error('Test Case ต้องมีอย่างน้อย 1 Output');
            }
...
function addMatchingPairRow() {
    ...
    // ดึงรายการคำถาม/คำตอบมาใส่ใน Dropdown
    const questions = document.querySelectorAll('#matchingQuestionsList .matching-question');
    const answers = document.querySelectorAll('#matchingAnswersList .matching-answer');
    
    // เช็คว่าสร้างคำถาม/ตอบหรือยัง
    if (questions.length === 0 || answers.length === 0) {
        alert("กรุณาสร้างคำถามและคำตอบด้านบนก่อนครับ");
        return;
    }
    ...
    div.style = "display: flex; gap: 10px; align-items: flex-end; margin-bottom: 10px; padding: 15px; background: #fff; border: 1px solid #ddd; border-radius: 6px;";
    
    div.innerHTML = `
        <div style="flex: 2;">
            <label style="font-size: 12px; font-weight:bold;">คำถาม</label>
            <select class="pair-question input-field" required style="width: 100%;">${qOptions}</select>
        </div>
        <div style="flex: 2;">
...
// ฟังก์ชันสำหรับเพิ่มแถวแนบลิงก์
function addAttachmentRow(data = null) {
    ...
    div.innerHTML = `
        <select class="attach-type input-field" style="width: 120px;">
        ...
        <input type="text" class="attach-url input-field" placeholder="วาง URL ที่นี่ (https://...)" value="${url}" style="flex: 2; min-width: 200px;">
        <button type="button" class="delete-btn" onclick="this.parentElement.remove()" style="padding: 8px 12px;">ลบ</button>
    `;
