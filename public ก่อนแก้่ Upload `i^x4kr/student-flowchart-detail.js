            ...
            descHTML = `
                <div class="problem-media" style="margin-bottom: 15px;">
                    <button type="button" class="primary-btn" onclick="openMediaModal('${studentProblemData.image}')" style="background-color: #17a2b8; border: none; padding: 8px 16px; border-radius: 4px; color: white; cursor: pointer; font-size: 14px; display: inline-flex; align-items: center; gap: 6px;">
                        <i class="fas fa-external-link-alt"></i> 📄 คลิกเพื่อดูสื่อประกอบ
                    </button>
                </div>
                ${descHTML}
            `;
    ...
    try {
        ...
        if (!snapshot.empty && isViewMode) {
            ...
            if (isViewMode) {
                ...
                // ซ่อนเฉพาะปุ่มตรวจคำตอบและล้าง canvas
...
async function submitAnswer() {
    ...
    try {
        ...
        if (submissionsSnapshot.empty || !submissionsSnapshot.docs[0].data().passed) {
            alert('กรุณาตรวจคำตอบและผ่านการตรวจก่อนส่งคำตอบ');
            return;
        }

        const lastValidSubmission = submissionsSnapshot.docs[0].data();
        ...
        await db.collection('submissions').add({
            ...
            maxScore: maxScore,
            details: lastValidSubmission.details,
            passed: true,
...
async function checkAnswer() {
    ...
    try {
        ...
        // คำนวณว่าผ่านหรือไม่
        const passed = Object.values(details).every(result => result === true);
...
function checkArrowsAndText(studentFlowchart, solutionFlowchart) {
    ...
    for (let i = 0; i < studentFlowchart.connections.length; i++) {
        const studentText = (studentFlowchart.connections[i].text || '').replace(/\s+/g, '');
        let foundMatch = false;
        
        // ค้นหา connection ที่ตรงกันในเฉลย
        for (let j = 0; j < solutionFlowchart.connections.length; j++) {
            const solutionText = (solutionFlowchart.connections[j].text || '').replace(/\s+/g, '');
...
function checkSymbolText(studentFlowchart, solutionFlowchart) {
    ...
    for (let i = 0; i < studentFlowchart.symbols.length; i++) {
        const studentText = (studentFlowchart.symbols[i].text || '').replace(/\s+/g, '').toLowerCase();
        let foundMatch = false;
        
        for (let j = 0; j < solutionFlowchart.symbols.length; j++) {
            const solutionText = (solutionFlowchart.symbols[j].text || '').replace(/\s+/g, '').toLowerCase();
            if (studentText === solutionText && 
...
window.openMediaModal = function(url) {
    ...
    if (lowerUrl.includes('youtube.com') || lowerUrl.includes('youtu.be')) {
        ...
        embedHtml = `<img src="${url}" style="max-width: 100%; max-height: 100%; object-fit: contain;">`;
    } else {
        embedHtml = `<iframe width="100%" height="100%" src="${url}" frameborder="0" allowfullscreen></iframe>`;
    }
