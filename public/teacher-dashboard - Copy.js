...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const textarea = document.getElementById('guiTemplate');
    
    // ลบ instance เดิมและสร้างใหม่
    if (window.editor) {
        window.editor.toTextArea(); // แปลงกลับเป็น textarea
    }

    // กำหนดค่า defaultCode ที่ใช้
    const defaultCode = `from tkinter import *
from tkinter import ttk

root = Tk()
root.title("โปรแกรมของฉัน00000000000")
...
root.mainloop()`;
    ...
    // ปรับข้อความใน textarea ให้ไม่มีการเยื้อง
    const template = document.getElementById('guiTemplate');
    template.value = defaultCode;
  ...
  `;
    ...
    // อัปเดตเนื้อหาเริ่มต้นเพื่อลบช่องว่างเกิน
    const initialCode = editor.getValue().trim(); // ลบช่องว่างท้ายและหัว
    editor.setValue(initialCode);

    setTimeout(() => {
        // ปรับค่าทุก cm-indent ให้เป็น 0
        document.querySelectorAll('.CodeMirror .cm-indent').forEach(el => {
          el.style.width = '0px';
          el.style.display = 'none';
        });
        
        // ปรับการแสดงผลของ content
        ...
        // ปรับการแสดงผลของ code mirror line
        document.querySelectorAll('.CodeMirror-line').forEach(line => {
          Array.from(line.children).forEach(child => {
            if (child.tagName.toLowerCase() === 'span') {
              ...
              // ลบการเยื้องใน token แรกของบรรทัด
    ...
    function fixCodeMirrorIndentation() {
        // เข้าถึงทุก DOM element ที่เกี่ยวข้องกับ CodeMirror
        ...
        // บังคับให้ editor refresh
        editor.refresh();
      }
      
      // เรียกใช้ฟังก์ชันหลังจาก CodeMirror ทำงานเสร็จ
      setTimeout(fixCodeMirrorIndentation, 100);
      
      // เรียกใช้ทุกครั้งที่ editor มีการเปลี่ยนแปลง
...
window.toggleProblemTypeFields = function () {
    ...
    const sections = ['pythonSection', 'comprehensionContentGroup',
        'questionsSection', 'matchingSection', 'flowchartSection','guiSection'];
...
function closeModal() {
    ...
    // ล้าง Preview และ Validation Message
    ...
    try {
        ...
        switch (problemType) {
            ...
            case 'matching':
                ...
                break;
            case 'gui':
                // เก็บข้อมูลสำหรับโจทย์ GUI
                problemData.description = document.getElementById('guiDescription')?.value?.trim() || '';
                if (!problemData.description) throw new Error('กรุณาใส่คำอธิบายโจทย์');
    
                problemData.templateCode = document.getElementById('guiTemplate')?.value?.trim() || '';
                if (!problemData.templateCode) throw new Error('กรุณาใส่โค้ดเริ่มต้น');
    
                // ตรวจสอบโค้ดก่อนบันทึก
                if (!validateGUICode(problemData.templateCode)) {
                    throw new Error('กรุณาแก้ไขโค้ดให้ถูกต้องตามข้อกำหนดก่อนบันทึก');
                }
                // รวบรวม Requirements
                const requirements = Array.from(document.querySelectorAll('.requirement-item'))
                    .map(item => {
                        ...
                        const props = item.querySelector('.requirement-props').value.trim();
                        const layout = item.querySelector('.requirement-layout')?.value || '';
                        const event = item.querySelector('.requirement-event')?.value.trim() || '';
                        const score = parseInt(item.querySelector('.requirement-score').value) || 5;
    
                        if (!type || !name || !props || isNaN(score) || score < 1 || score > 10) {
                            throw new Error('กรุณากรอกข้อมูลข้อกำหนดให้ครบถ้วนและคะแนนต้องอยู่ในช่วง 1-10');
                        }
    
                        return { type, name, props, layout, event, score };
                    });
    
                if (requirements.length === 0) {
                    throw new Error('กรุณากำหนด Widgets อย่างน้อย 1 รายการ');
                }
    
                // ตรวจสอบว่าไม่มีชื่อ Widget ซ้ำกัน
                const names = requirements.map(req => req.name);
                if (new Set(names).size !== names.length) {
                    throw new Error('ชื่อ Widget ต้องไม่ซ้ำกัน');
                }
    
                problemData.requirements = requirements;
                problemData.guiSettings = {
                    title: problemData.templateCode.match(/title\(['"](.*?)['"]\)/)?.[1] || 'โปรแกรมของฉัน',
                    geometry: problemData.templateCode.match(/geometry\(['"](\d+x\d+)['"]\)/)?.[1] || '400x300'
                };

                // เก็บข้อมูลภาพประกอบ (ถ้ามี)
                const imageUrl = document.getElementById('guiImageUrl').value.trim();
                if (imageUrl) {
                    problemData.guiImage = imageUrl;
                }
                break;    
        }
...
function previewGUI() {
    const code = document.getElementById('guiTemplate').value;
...
function addGUIRequirement() {
    ...
    item.innerHTML = `
        ...
        <input type="text" class="requirement-name" placeholder="ชื่อ widget (เช่น btn_calculate)" required>
        <input type="text" class="requirement-props" placeholder="properties (เช่น bg='blue', fg='white')" required>
        <select class="requirement-layout" style="display: none;">
            <option value="pack">pack</option>
            <option value="grid">grid</option>
            <option value="place">place</option>
        </select>
        <input type="text" class="requirement-event" placeholder="Event (เช่น command=some_function)" style="display: none;">
        <input type="number" class="requirement-score" value="5" min="1" max="10" required>
        <button type="button" class="delete-requirement-btn" onclick="deleteRequirement(this)">ลบ</button>
    `;
    container.appendChild(item);

    // เพิ่ม event listener สำหรับแสดง/ซ่อน Layout และ Event ตามประเภท Widget
    const typeSelect = item.querySelector('.requirement-type');
    const layoutSelect = item.querySelector('.requirement-layout');
    const eventInput = item.querySelector('.requirement-event');

    typeSelect.addEventListener('change', () => {
        const type = typeSelect.value;
        if (['Button', 'Label', 'Entry', 'Frame'].includes(type)) {
...
function deleteRequirement(button) {
    if (confirm('คุณแน่ใจหรือไม่ที่จะลบข้อกำหนดนี้?')) {
        button.parentElement.remove();
    }
}
function validateGUICode(code) {
    const validationMessage = document.getElementById('guiValidationMessage');
    try {
        // ตรวจสอบว่ามี root = Tk()
        if (!code.includes('root = Tk()')) {
            throw new Error('โค้ดต้องมี root = Tk() เพื่อสร้างหน้าต่างหลัก');
        }

        // ตรวจสอบว่ามี root.mainloop()
        if (!code.includes('root.mainloop()')) {
            throw new Error('โค้ดต้องมี root.mainloop() เพื่อรันโปรแกรม');
        }

        // ตรวจสอบ Widgets ตาม Requirements
        const requirements = Array.from(document.querySelectorAll('.requirement-item'))
            .map(item => item.querySelector('.requirement-type').value);

        requirements.forEach(req => {
            if (!code.includes(req)) {
                throw new Error(`โค้ดต้องมี Widget ${req} ตามข้อกำหนด`);
            }
        });

        validationMessage.className = 'validation-message success';
        validationMessage.textContent = 'โค้ดถูกต้องตามข้อกำหนดทั้งหมด!';
