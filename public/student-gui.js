...
// รับ problemData ที่ดึงมาแล้วจาก loadGUIProblem() เป็นพารามิเตอร์ที่ 2 (optional)
// เพื่อเลี่ยงการยิง Firestore ซ้ำไปดึง problems/{problemId} เอกสารเดิมอีกรอบ
// (เดิม loadGUIProblem ดึงเอกสารนี้ไปแล้ว แต่ loadProblemConfig ก็ยังดึงซ้ำอีกที เสีย round-trip ฟรี)
// ถ้าไม่ส่ง data มา (เผื่อมีที่อื่นเรียกใช้แบบเดิม) จะ fallback ไปดึงเองเหมือนพฤติกรรมเดิม
async function loadProblemConfig(problemId, preloadedData = null) {
    const data = preloadedData || (await db.collection('problems').doc(problemId).get()).data();
...
async function loadGUIProblem(problemId, userId, classId, viewMode) {
    try {
        ...
        // ส่ง problemData ที่ดึงมาแล้วด้านบนไปใช้ซ้ำ แทนที่จะให้ loadProblemConfig ไปดึง
        // problems/{problemId} จาก Firestore ซ้ำอีกรอบ (ลด 1 round-trip ต่อการเปิดโจทย์ 1 ครั้ง)
        await loadProblemConfig(problemId, problemData);
