...
function doGet() {
    return HtmlService.createTemplate(getHtmlContent())
      .evaluate()
      .setTitle('งานวิ่ง 100 ปี ถาวรานุกูล')
    ...
    try {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (orderId.includes(searchLower) || 
          ...
          // คำนวณยอดการซื้อ
          const distance = (row[7] !== null && row[7] !== undefined) ? row[7].toString() : '';
          const deliveryMethod = (row[14] !== null && row[14] !== undefined) ? row[14].toString() : '';
          
          let amount = 0;
          
          // คำนวณราคาตามระยะทาง
          if (distance.includes('3') || distance.toLowerCase().includes('กิโล')) {
            amount = 500;
          } else if (distance.toLowerCase().includes('vip')) {
            amount = 1000;
          }
          
          // เพิ่มค่าจัดส่ง 60 บาท สำหรับทุก order ที่เลือกจัดส่ง
          ...
          results.push(result);
          console.log('พบข้อมูลที่ตรงกัน:', result.name, result.surname, 'อายุ:', realAge, 'ยอดเงิน:', amount);
        }
      }
  ...
  <html>
  <head>
      <title>งานวิ่ง 100 ปี ถาวรานุกูล</title>
      <meta charset="UTF-8">
...
@keyframes badgeGlow {
  ...
  <body>
      <div class="container">
          <div class="header">
              <img src="https://drive.google.com/uc?export=view&id=YOUR_LOGO_FILE_ID" alt="Logo ทีมแดงขาว" class="logo" onerror="this.style.display='none'">
              <h1>🏃‍♂️ งานวิ่ง 100 ปี 🏃‍♀️</h1>
             <h2>ถาวรานุกูล️</h2>
              <div class="team-badge">⚪ ทีมแดงขาว 🥈🥇 ⚪</div>
              <div class="subtitle">ระบบตรวจสอบสถานะการสมัครและข้อมูลผู้เข้าร่วม</div>
          </div>
      ...
      <script>
          ...
          function displayResults(results) {
              ...
              if (results.length === 0) {
                  resultsDiv.innerHTML = '<div class="no-results">❌ ไม่พบข้อมูลที่ค้นหา<br><small>กรุณาตรวจสอบการสะกดหรือลองค้นหาด้วยข้อมูลอื่น</small></div>';
                  return;
              }
              ...
              html += '<table>';
              html += '<thead><tr>';
              html += '<th style="min-width: 120px;">🆔 รหัสการซื้อ</th>';
              html += '<th style="min-width: 100px;">👤 ชื่อผู้แข่ง</th>';
              html += '<th style="min-width: 100px;">👤 นามสกุล</th>';
              html += '<th style="min-width: 60px;">⚧ เพศ</th>';
              html += '<th style="min-width: 120px;">📊 ช่วงอายุที่ลงแข่ง</th>';
              html += '<th style="min-width: 80px;">🎂 อายุจริง (ปี)</th>';
              html += '<th style="min-width: 100px;">🏃 ระยะทาง</th>';
              ...
              results.forEach((result, index) => {
                  ...
                  html += '<td class="age-cell">' + (result.realAge || '-') + '</td>';
                  html += '<td>' + (result.distance || '-') + '</td>';
                  html += '<td>' + (result.deliveryMethod || '-') + '</td>';
                  html += '<td class="amount-cell">' + (result.amount ? result.amount + ' ฿' : '-') + '</td>';
