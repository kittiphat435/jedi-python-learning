...
async function saveProblem(event) {
    ...
    try {
        if (problemType === 'gui') {
            ...
            const solutionCode = ''; // teacher-dashboard.html ไม่มีฟิลด์สำหรับเฉลยใน python
...
function renderProblemList(problems) {
    ...
    if (problems.length === 0) {
        problemList.innerHTML = '<p>ไม่พบโจทย์ที่ตรงกับเงื่อนไข</p>';
        return;
    }
...
window.updateAttachmentPreview = function(element) {
    ...
    // ซ่อนพรีวิวเก่าเวลาเปลี่ยนประเภท
...
window.previewAttachment = function(btnElement) {
    ...
    if (type === 'image') {
        ...
        // PDF หรือ Link ให้เปิดหน้าต่างใหม่
...
async function handleGuiImageUpload(event) {
    ...
    try {
        ...
        if(typeof previewGUIImage === 'function') previewGUIImage();
