...
class FlowchartEditor {
    ...
    createShape(type) {
        ...
        if (shape) {
            this.setBasicShapeAttributes(shape, type);
        }
        return shape;
    }
    ...
    createDisplay(width, height) {
        const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
        const triangleWidth = width * 0.2; // ความกว้างของสามเหลี่ยมด้านซ้าย
        const curveDepth = width * 0.15;   // ความลึกของส่วนโค้งด้านขวา
        
        // รูป Display:
        // - ด้านซ้ายแหลมเป็นสามเหลี่ยม (เข้าหาจุดศูนย์กลาง)
        // - ด้านขวาโค้งนูน
        const d = `M ${-width / 2 + triangleWidth} ${-height / 2}
                   L ${width / 2 - curveDepth} ${-height / 2}
                   Q ${width / 2 + curveDepth} ${-height / 2} ${width / 2 + curveDepth} 0
                   Q ${width / 2 + curveDepth} ${height / 2} ${width / 2 - curveDepth} ${height / 2}
                   L ${-width / 2 + triangleWidth} ${height / 2}
                   L ${-width / 2} 0 Z`;
    ...
    createDataStorage(width, height) {
        ...
        // โค้งนูนออกด้านซ้าย โค้งเว้าเข้าด้านขวา (เหมือนสัญลักษณ์ Stored Data มาตรฐาน)
        const d = `M ${-width / 2 + arcWidth} ${-height / 2}
                   L ${width / 2} ${-height / 2}
                   Q ${width / 2 - arcWidth} ${-height / 2} ${width / 2 - arcWidth} 0
                   Q ${width / 2 - arcWidth} ${height / 2} ${width / 2} ${height / 2}
                   L ${-width / 2 + arcWidth} ${height / 2}
                   Q ${-width / 2 - arcWidth} ${height / 2} ${-width / 2 - arcWidth} 0
                   Q ${-width / 2 - arcWidth} ${-height / 2} ${-width / 2 + arcWidth} ${-height / 2} Z`;
        path.setAttribute('d', d);
        return path;
    }
    ...
    setBasicShapeAttributes(shape, type) {
        let fillColor = 'white';
        if (type) {
            switch (type) {
                case 'start-end': fillColor = '#fce4ec'; break; // Light Pink
                case 'process': fillColor = '#e3f2fd'; break; // Light Blue
                case 'decision': fillColor = '#fff9c4'; break; // Light Yellow
                case 'input-output': fillColor = '#e8f5e9'; break; // Light Green
                case 'manual-input': fillColor = '#e0f7fa'; break; // Light Cyan
                case 'display': fillColor = '#f3e5f5'; break; // Light Purple
                case 'document': fillColor = '#fff3e0'; break; // Light Orange
                case 'data-storage': fillColor = '#eceff1'; break; // Light Grey
                case 'connector': fillColor = '#fbe9e7'; break; // Light Peach
            }
        }
        
        // ถ้าถูกเรียกโดยส่ง type มา หรือยังไม่มีสี ให้ใส่สี
        if (type || !shape.hasAttribute('fill')) {
                ...
                if (!existingConnection) {
                    ...
                    if (sourceSymbol && targetSymbol && sourcePoint && targetPoint) {
                        ...
                        const connectionData = {
                            ...
                            text: (textElement && textElement.textContent !== 'ดับเบิลคลิกเพื่อพิมพ์' && textElement.textContent !== 'พิมพ์ valua' && textElement.textContent !== 'พิมพ์ value') ? textElement.textContent : '',
                    ...
                    if (sourceEl && targetEl) {
                        ...
                        if (sourcePoint && targetPoint) {
                            ...
                            if (connection) {
                                ...
                                if (textEl && conn.text && conn.text !== 'พิมพ์ valua' && conn.text !== 'พิมพ์ value' && conn.text !== 'ดับเบิลคลิกเพื่อพิมพ์') {
                ...
                if (data.connections && Array.isArray(data.connections)) {
                    ...
                    data.connections.forEach(connData => {
                        ...
                        if (connection) {
                            ...
                            if (textElement && connData.text && connData.text !== 'พิมพ์ valua' && connData.text !== 'พิมพ์ value' && connData.text !== 'ดับเบิลคลิกเพื่อพิมพ์') {
                    ...
                    data.connections.forEach(conn => {
                        ...
                        if (sourceSymbol && targetSymbol) {
                            ...
                            if (sourcePoint && targetPoint) {
                                ...
                                // Add text if exists
                                if (conn.text && conn.text !== 'พิมพ์ valua' && conn.text !== 'พิมพ์ value' && conn.text !== 'ดับเบิลคลิกเพื่อพิมพ์') {
                                    const textGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
                                    textGroup.setAttribute('class', 'text-group');
