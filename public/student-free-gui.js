...
def check_static_names(source_code):
    try:
    ...
    tkinter_names = ['Tk', 'Label', 'Button', 'Entry', 'StringVar', 'IntVar', 'Checkbutton', 'Frame', 
                     'Canvas', 'Text', 'Scrollbar', 'Listbox', 'Radiobutton', 'Scale', 'Spinbox', 
                     'Menu', 'Menubutton', 'Toplevel', 'PanedWindow', 'LabelFrame', 'PhotoImage', 'BitmapImage',
                     'W', 'E', 'N', 'S', 'END', 'CENTER', 'LEFT', 'RIGHT', 'TOP', 'BOTTOM', 'YES', 'NO', 'X', 'Y', 'BOTH', 
                     'filedialog', 'messagebox', 'ttk']
    math_names = ['sin', 'cos', 'tan', 'sqrt', 'pi', 'math', 'random']
    defined_names.update(tkinter_names + math_names)
...
json.dumps(check_static_names(student_code_to_check))
`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        if (problemId) {
            loadingTasks.push(loadGUIProblem(problemId, targetStudentId, classId, viewMode));
            
            // ตรวจสอบสถานะการส่งงาน
            if (!viewMode) {
                loadingTasks.push(checkSubmissionStatus(problemId, targetStudentId));
            }
        }
        ...
        // Event Listeners
        setupEventListeners(problemId, classId, targetStudentId, viewMode);
...
async function loadGUIProblem(problemId, userId, classId, viewMode) {
    try {
        ...
        // Ensure the problem is of type 'free_gui'
        if (problemData.type !== 'free_gui') {
            showError('โจทย์นี้ไม่ใช่โจทย์ GUI (อิสระ)');
            return;
        }

        // สำหรับ free_gui ไม่มีการคิดคะแนนจาก test case
        guiBaseResult = { score: 1, maxScore: 1, passed: true };
        guiTestCaseBonus = { score: 0, maxScore: 0 };
        updateScoreDisplay(1, 1);
        ...
        if (problemData.widgets && Array.isArray(problemData.widgets)) {
            ...
            if (reqList) reqList.innerHTML = '<p>ไม่มี Widgets ระบุ</p>';
        }
...
function setupEventListeners(problemId, classId, userId, viewMode) {
    ...
    // --- Event Listeners เดิม ---
      if (checkBtn && !viewMode) {
          checkBtn.addEventListener('click', async () => {
              const code = codeEditor.value;
              const isValid = await checkGUICode(code);
              if (convertBtn) convertBtn.disabled = !isValid;
              if (submitBtn) submitBtn.disabled = !isValid; // เปิดปุ่มส่งงานเมื่อโค้ดถูกต้อง
          });
      }
    ...
    if (convertBtn) {
        convertBtn.addEventListener('click', async () => {
            ...
            // 2. ✅ ในโหมด View อนุญาตให้รันได้เลยโดยไม่ต้องสน Syntax ถ้ามั่นใจ หรือเช็คขำๆ
            let isValid = true;
            if (!viewMode) {
                isValid = await checkGUICode(code);
            }
            ...
            if (isValid) {
                ...
                // 5. ✅✅ บันทึกโค้ดทันที (ทุกกรณีที่ไม่ Error) ยกเว้นโหมด View
                if (!viewMode) {
    ...
    if (submitBtn && !viewMode) {
        submitBtn.addEventListener('click', async () => {
            ...
            // อนุญาตให้ส่งซ้ำได้เสมอสำหรับ free_gui
...
async function submitGUICode(code, problemId, userId, classId) {
    ...
    try {
        ...
        await db.collection('submissions').add({
            ...
            status: 'completed',
            testResult: 'passed',
            score: 1, // หรือใส่ 0 หรือค่าเริ่มต้นสำหรับงานที่ไม่คิดคะแนน
            maxScore: 1
        });
...
async function sendToSimulator(autoRun = false) {
    ...
    try {
      ...
      // **เพิ่ม onload ก่อนกำหนดค่า srcdoc เพื่อป้องกันปัญหา Race Condition และกำหนด Timeout ให้ Render ทัน**
      resultFrame.onload = () => {
        setTimeout(() => {
          try {
...
<body>
    ...
    functions.forEach(fn => {
      js += `async function ${fn.name}(${fn.args || ''}) {\n`;
      ...
      fn.lines.forEach(fl => {
        ...
        // database append: db.append([var1, var2])
        if (m = line.match(/^([\p{L}_][\p{L}\p{N}_]*)\.append\s*\(\s*\[(.*?)\]\s*\)$/u)) {
            let dbVar = m[1];
            let rawArgs = m[2];
            let parsedArgs = rawArgs.split(',').map(arg => {
