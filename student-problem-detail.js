...
function displayTestResults(results) {
    ...
    console.log('All passed:', allPassed); // เพิ่ม log

    container.innerHTML = `
        <div class="test-summary ${allPassed ? 'success' : 'failure'}">
            <h3>ผลการทดสอบ: ผ่าน ${results.filter(r => r.passed).length} จาก ${results.length} test cases</h3>
            ${allPassed ? '✅ ยินดีด้วย! ผ่านทุก test cases' : '❌ ยังไม่ผ่านทุก test cases'}
        </div>
        ${results.map((result, index) => `
            <div class="test-case-result ${result.passed ? 'test-case-success' : 'test-case-failure'}">
        ...
        ${allPassed ? `
            <div class="submit-section" style="margin-top: 20px; text-align: center;">
                <button onclick="submitToTeacher()" class="primary-btn">ส่งงานให้ครูตรวจ</button>
            </div>
        ` : ''}
    `;
}
    ...
    try {
        ...
        const results = await Promise.all(currentProblem.testCases.map(async (testCase) => {
            try {
                ...
                // ทำความสะอาดและเปรียบเทียบผลลัพธ์
                ...
                // แยกบรรทัดและเอาบรรทัดสุดท้าย
                const lines = actualOutput.split('\n');
                const lastLine = lines[lines.length - 1];
                // เอาเฉพาะส่วนที่อยู่หลังเครื่องหมาย : ตัวสุดท้าย ถ้ามี
                ...
                return {
                    ...
                    debug: {
                        ...
                        cleanActual,
                        length: {
                            expected: cleanExpected.length,
                            actual: cleanActual.length
                        }
                    }
                };
            } catch (error) {
        ...
        // เช็คว่าผ่านทุกข้อไหม
        const allPassed = results.every(r => r.passed);

        // อัพเดทสถานะถ้าผ่านทุกข้อ
        if (allPassed) {
...
function resetCode() {
    if (confirm('คุณแน่ใจหรือไม่ที่จะรีเซ็ตโค้ด?')) {
        document.getElementById('codeEditor').value = currentProblem?.templateCode || '# เขียนโค้ดของคุณที่นี่';
...
async function submitToTeacher() {
    try {
        ...
        let classId = params.get('classId'); // พยายามดึง classId จาก URL ก่อน

        // ถ้าไม่มี classId ใน URL ลองดึงจาก currentProblem
        if (!classId && currentProblem?.classId) {
            classId = currentProblem.classId;
        }

        // ถ้ายังไม่มี classId ลองดึงจาก document.referrer
        if (!classId && document.referrer) {
            const referrerUrl = new URL(document.referrer);
            const referrerParams = referrerUrl.searchParams;
            if (referrerParams.get('id')) {
        ...
        if (!classId) {
            // ถ้ายังไม่มี classId ลองดึงจาก problems collection
            const problemRef = await db.collection('problems').doc(problemId).get();
            if (problemRef.exists && problemRef.data().classId) {
                classId = problemRef.data().classId;
            }
        }

        // ถ้ายังไม่มี classId อีก
        if (!classId) {
