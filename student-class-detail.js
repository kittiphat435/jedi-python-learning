...
async function loadProblems(classId, userId) {
    ...
    try {
        ...
        // Fetch submissions with classId included
        ...
        div.innerHTML = `
            <div class="problem-info">
                <div class="problem-header">
                    ...
                    <span class="status-badge ${problem.status}-badge">${problem.status === 'completed' ? 'สำเร็จ' : 'ยังไม่ได้ทำ'}</span>
                </div>
                <h3>${problem.title}</h3>
                <p>${contentToShow}</p>
            </div>
            <button onclick="viewProblem('${problem.id}', '${problem.type}')" class="primary-btn">
                ${problem.status === 'completed' ? 'ดูผลลัพธ์' : 'ทำโจทย์'}
            </button>
        `;
...
async function loadStats(classId, userId) {
    try {
        // จำนวนโจทย์ทั้งหมดที่ไม่ซ้ำกัน
        const classProblemsSnapshot = await db.collection('class_problems')
            .where('classId', '==', classId)
            .get();

        // ใช้ Set เพื่อนับจำนวนโจทย์ที่ไม่ซ้ำกัน
        ...
        // จำนวนโจทย์ที่ทำสำเร็จ (ไม่ซ้ำ)
        const submissionsSnapshot = await db.collection('submissions')
            .where('classId', '==', classId)
            .where('studentId', '==', userId)
            .get();

        // เก็บเฉพาะ submission ล่าสุดของแต่ละโจทย์
        ...
        submissionsSnapshot.forEach(doc => {
            ...
            // ถ้ายังไม่มี submission ของโจทย์นี้ หรือ submission นี้ใหม่กว่า
            if (!latestSubmissions.has(data.problemId) ||
                data.submittedAt.toDate() > latestSubmissions.get(data.problemId).submittedAt.toDate()) {
                latestSubmissions.set(data.problemId, currentSubmission);
            }
        });

        // นับจำนวนโจทย์ที่ทำสำเร็จจาก submission ล่าสุดเท่านั้น
        const completedProblems = Array.from(latestSubmissions.values())
        ...
        console.log('Stats Debug:', {
            totalProblems,
            uniqueProblemIds: Array.from(uniqueProblemIds),
            latestSubmissions: Array.from(latestSubmissions.entries()),
            completedProblems,
            progress
        });
...
function viewProblem(problemId, type) {
    const url = type === 'python'
        ? `student-problem-detail.html?id=${problemId}`
        : `student-quiz-detail.html?id=${problemId}`;
