...
async function viewProgress(studentId) {
    try {
        ...
        const classProblemsSnapshot = await db.collection('class_problems')
            .where('classId', '==', classId)
            .get();
        console.log('Found class problems:', classProblemsSnapshot.size);
        ...
        for (const doc of classProblemsSnapshot.docs) {
            const problemDoc = await db.collection('problems').doc(doc.data().problemId).get();
            if (problemDoc.exists) {
                ...
                console.log(`Problem ${doc.data().problemId}:`, {
                    type: data.type,
                    maxScore: maxScore
                });

                problemDetails.set(doc.data().problemId, {
                    ...data,
                    maxScore
                });
            }
