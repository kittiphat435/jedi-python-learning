class FlowchartChecker {
    ...
    isSymbolMatch(symbol1, symbol2) {
        ...
        return this.calculateTextSimilarity(
            symbol1.text.toLowerCase(),
            symbol2.text.toLowerCase()
        ) >= 0.8;
    }

    isConnectionTextMatch(conn1, conn2) {
        if (!conn1.text && !conn2.text) return true;
        if (!conn1.text || !conn2.text) return false;
        return this.calculateTextSimilarity(
            conn1.text.toLowerCase(),
            conn2.text.toLowerCase()
        ) >= 0.8;
    }
