...
function formatDistance(distance) {
  ...
  // ฟังก์ชันแยกข้อมูลระยะทางและช่วงอายุ (ปรับปรุงใหม่)
  function parseAgeGroupData(ageGroupText) {
    ...
    const text = ageGroupText.toString().trim().toLowerCase();
    
    // แยกระยะทาง - ใช้ regular expression ที่แม่นยำกว่า
    if (/\b10\s*(km|กิโลเมตร|กม|กิโล)\b/i.test(text)) {
      distance = '10 Km';
    } else if (/\b5\s*(km|กิโลเมตร|กม|กิโล)\b/i.test(text)) {
      distance = '5 Km';
    } else if (/\b3\s*(km|กิโลเมตร|กม|กิโล)\b/i.test(text)) {
      distance = '3 Km';
    }
    
    // แยกช่วงอายุ - เพิ่มรูปแบบที่หลากหลายมากขึ้น
    if (text.includes('ไม่จำกัด') || text.includes('ไม่จํากัด') || text.includes('ไม่จากัด')) {
      ageRange = 'อายุไม่จำกัด';
    } else if (text.includes('ไม่เกิน 15') || text.includes('ไม่เกิน15') || text.includes('15 ปี') || text.includes('15ปี')) {
      ageRange = 'อายุ ไม่เกิน 15 ปี';
    } else if (text.includes('ไม่เกิน 18') || text.includes('ไม่เกิน18') || text.includes('18 ปี') || text.includes('18ปี')) {
      ageRange = 'อายุ ไม่เกิน 18 ปี';
    } else if ((text.includes('16') && text.includes('29')) || text.includes('16-29') || text.includes('16 - 29')) {
      ageRange = 'อายุ 16-29 ปี';
    } else if ((text.includes('30') && text.includes('39')) || text.includes('30-39') || text.includes('30 - 39')) {
      ageRange = 'อายุ 30 - 39 ปี';
    } else if ((text.includes('40') && text.includes('49')) || text.includes('40-49') || text.includes('40 - 49')) {
      ageRange = 'อายุ 40 - 49 ปี';
    } else if ((text.includes('50') && text.includes('59')) || text.includes('50-59') || text.includes('50 - 59')) {
      ageRange = 'อายุ 50 - 59 ปี';
    } else if ((text.includes('60') && text.includes('69')) || text.includes('60-69') || text.includes('60 - 69')) {
      ageRange = 'อายุ 60 - 69 ปี';
    } else if (text.includes('70') && (text.includes('ขึ้นไป') || text.includes('ขึ้น') || text.includes('+'))) {
      ageRange = 'อายุ 70 ปี ขึ้นไป';
    }
    
    // Log เพื่อ debug
    Logger.log(`parseAgeGroupData: Input="${ageGroupText}", Distance="${distance}", AgeRange="${ageRange}"`);
  ...
  function onFormSubmit(e) {
    ...
    // เพิ่ม Logger เพื่อ debug
    Logger.log(`onFormSubmit: Row ${row}, AgeGroupData from column G: "${ageGroupData}"`);
    
    const parsedData = parseAgeGroupData(ageGroupData);
    
    Logger.log(`onFormSubmit: Parsed Distance="${parsedData.distance}", AgeRange="${parsedData.ageRange}"`);
    ...
    Logger.log(`onFormSubmit: Successfully updated columns U and V for row ${row}`);
    
    // เพิ่มการอัปเดตลำดับในคอลัมน์ T
  ...
  // ฟังก์ชันประมวลผลข้อมูลทั้งหมดเมื่อโหลดหน้าเว็บ
  function processAllDataOnLoad() {
    try {
      ...
      const lastRow = sheet.getLastRow();
      if (lastRow < 2) {
        Logger.log('ไม่มีข้อมูลในชีต');
        return;
      }
  
      // 1. อัปเดตลำดับในคอลัมน์ T
      updateSequenceNumbers(sheet);
      
      // 2. แยกข้อมูลระยะวิ่งและรุ่นวิ่งในคอลัมน์ U และ V
      const ageGroupData = sheet.getRange(2, 7, lastRow - 1, 1).getValues();
      
      // เพิ่มหัวข้อในคอลัมน์ U และ V ถ้ายังไม่มี
      const headerU = sheet.getRange(1, 21).getValue();
      const headerV = sheet.getRange(1, 22).getValue();
      
      if (!headerU || headerU.toString().trim() === '') {
        sheet.getRange(1, 21).setValue('ระยะวิ่ง');
      }
      if (!headerV || headerV.toString().trim() === '') {
        sheet.getRange(1, 22).setValue('รุ่นวิ่ง');
      }
      ...
      Logger.log(`ประมวลผลข้อมูลทั้งหมดเสร็จสิ้น: ลำดับ + แยกข้อมูล ${processedCount} แถว + ยอดรวม`);
      
    } catch (error) {
      Logger.log('Error processing all data on load: ' + error.message);
    }
  }
  ...
  function getOrdersHtml() {
    try {
      ...
      // ประมวลผลข้อมูลทั้งหมดเมื่อโหลดหน้าเว็บ
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        // เพิ่มการจัดรูปแบบสีสำหรับสถานะ
                  let statusClass = '';
                  let statusDisplay = status;
                  if (status === 'การสมัครเสร็จสิ้น') {
                    statusClass = 'status-success';
                  } else if (status === 'สมัครไม่สำเร็จ') {
  ...
  function confirmOrder(rowIndex) {
    try {
      ...
      if (rowIndex >= 1 && rowIndex < data.length) {
        ...
        // ไม่ต้องเรียก processExistingDataForUV() เพราะข้อมูล U & V ถูกประมวลผลแล้วตอน submit form
        // processExistingDataForUV() จะถูกเรียกอัตโนมัติใน onFormSubmit()
  ...
  // ฟังก์ชันสำหรับบันทึกลำดับในคอลัมน์ T (รันแยกต่างหาก)
  function updateSequenceNumbersManual() {
    try {
      const sheet = SpreadsheetApp.openById('1CmLJs8fOu5IFuW4dROJ2xfcVlrwcLZpoQSsqdb0W6UM').getSheetByName('Sheet1');
      if (!sheet) {
        Logger.log('ไม่พบ Sheet ชื่อ Sheet1');
      ...
      if (lastRow > 1) {
        ...
        Logger.log(`อัปเดตลำดับเสร็จสิ้น: ${sequenceData.length} แถว`);
        return `อัปเดตลำดับเสร็จสิ้น: ${sequenceData.length} แถว`;
      } else {
        return 'ไม่มีข้อมูลในชีต';
      }
    } catch (error) {
      Logger.log('Error updating sequence numbers manually: ' + error.message);
      return 'เกิดข้อผิดพลาด: ' + error.message;
    }
  }
  
  // ฟังก์ชันสำหรับแบ่งระยะวิ่งในคอลัมน์ U และรุ่นวิ่งในคอลัมน์ V (รันแยกต่างหาก)
  function updateDistanceAndAgeGroupManual() {
    try {
      const sheet = SpreadsheetApp.openById('1CmLJs8fOu5IFuW4dROJ2xfcVlrwcLZpoQSsqdb0W6UM').getSheetByName('Sheet1');
      if (!sheet) {
        Logger.log('ไม่พบ Sheet ชื่อ Sheet1');
      ...
      // เพิ่มหัวข้อในคอลัมน์ U และ V ถ้ายังไม่มี
      const headerU = sheet.getRange(1, 21).getValue(); // คอลัมน์ U = 21
      const headerV = sheet.getRange(1, 22).getValue(); // คอลัมน์ V = 22
      ...
      for (let i = 0; i < ageGroupData.length; i++) {
        ...
        if (ageGroupText && ageGroupText !== '-' && ageGroupText.toString().trim() !== '') {
          ...
          // Log เพื่อ debug
          Logger.log(`Row ${rowIndex}: "${ageGroupText}" -> Distance: "${parsedData.distance}", AgeRange: "${parsedData.ageRange}"`);
        }
      }
      
      Logger.log(`แบ่งข้อมูลระยะวิ่งและรุ่นวิ่งเสร็จสิ้น: ${processedCount} แถว`);
      return `แบ่งข้อมูลระยะวิ่งและรุ่นวิ่งเสร็จสิ้น: ${processedCount} แถว`;
      
    } catch (error) {
      Logger.log('Error updating distance and age group manually: ' + error.message);
      return 'เกิดข้อผิดพลาด: ' + error.message;
    }
  }
  
  // ฟังก์ชันรวมสำหรับรันทั้งสองฟังก์ชันพร้อมกัน
  function updateAllColumnsManual() {
    try {
      const sequenceResult = updateSequenceNumbersManual();
      const distanceAgeResult = updateDistanceAndAgeGroupManual();
      
      const combinedResult = `ผลลัพธ์การอัปเดต:\n- ${sequenceResult}\n- ${distanceAgeResult}`;
      Logger.log(combinedResult);
      return combinedResult;
      
    } catch (error) {
      Logger.log('Error updating all columns manually: ' + error.message);
      return 'เกิดข้อผิดพลาด: ' + error.message;
    }
  }