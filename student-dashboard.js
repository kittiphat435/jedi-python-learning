    ...
    try {
        ...
        if (user.photoURL) {
            try {
                ...
                console.warn('Unable to load profile image, using default avatar', error);
            }
        }
                ...
                <div class="profile-container">
                    ...
                    <div class="profile-details">
                        <div class="profile-name-container">
                            <span id="displayName">${user.displayName || 'นักเรียน'}</span>
                        ...
                        <div id="editNameForm" style="display: none;">
                            <input type="text" id="newName" class="edit-name-input" 
                                   value="${user.displayName || 'นักเรียน'}"
...
async function loadStats(userId) {
    try {
        ...
        // นับจำนวนโจทย์ทั้งหมดจาก class_problems
        let totalProblems = 0;
        if (classIds.length > 0) {
            ...
            // ใช้ Set เพื่อนับเฉพาะ problemId ที่ไม่ซ้ำกัน
        ...
        // จำนวนโจทย์ที่ทำสำเร็จ - ใช้ submissions collection
        const submissionsSnapshot = await db.collection('submissions')
            .where('studentId', '==', userId)
            .where('status', '==', 'completed')
            .get();

        // นับเฉพาะโจทย์ที่ไม่ซ้ำกัน
        ...
        console.log('Stats:', {
            ...
            completedProblems,
            classIds,
            uniqueCompletedProblems: Array.from(completedProblemIds)
        });

    } catch (error) {
        console.error("Error loading stats:", error);
        // แสดงค่าเป็น 0 เมื่อเกิดข้อผิดพลาด
